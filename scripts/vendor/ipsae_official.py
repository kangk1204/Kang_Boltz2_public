...
if boltz:
    ...
    if os.path.exists(summary_file_path):
        with open(summary_file_path, 'r') as file:
            ...
            # LOCAL PATCH (2026-09-18): fallback {} 선언 후 곧바로 원래 key 를 다시 조회해
            # KeyError 가 나던 문제 수정. 누락된 chain pair 는 0 으로 둔다.
            # boltz_chain_pair_iptm_data=data_summary['pair_chains_iptm']
            for nchain1, chain1 in enumerate(unique_chains):
                for nchain2, chain2 in enumerate(unique_chains):
                    if chain1 == chain2: continue
                    pair_data = boltz_chain_pair_iptm_data.get(str(nchain1), {})
                    iptm_boltz[chain1][chain2] = float(pair_data.get(str(nchain2), 0.0))
    else:
